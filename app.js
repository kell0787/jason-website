let arr = ["fa fa-diamond", "fa fa-diamond", "fa fa-paper-plane-o", "fa fa-paper-plane-o", "fa fa-anchor", "fa fa-anchor", "fa fa-bolt", "fa fa-bolt", "fa fa-cube", "fa fa-cube", "fa-leaf", "fa-leaf", "fa fa-bicycle", "fa fa-bicycle", "fa fa-bomb", "fa fa-bomb"]; //Array of all possible cards
...
function restart () { //When someone clicks the RESET button.
  ...
  winCounter = 0;       // Win rating reset.
    while(removedCounter != 0){  //3 Star rating reeset
  ...
  for (let j = 0; j < 16; j++) {  //Code for adding 16 cards back onto the page.
...
function respondToTheClick (evt) {
  if (evt.target.className != "card open show" && evt.target.className != "card match" && stopClicks) { //So players cannot click the same card twice or spam click to break the game.
    counter ++;
    timer();
      if (moves.textContent == 3 || moves.textContent == 28) { //Removing Stars at higher moves
        if (counter == 1) {
        removedCounter ++;
    ...
    evt.target.className = "card open show";
      if (counter == 1) {                   //First Click
        ...
        counter = 0;
        if (winCounter == 16) {      //Winner winner chicken dinner
          stop();
          setTimeout(function() {  //Win screen appears
            document.getElementById('win').style.display = "flex";
            document.getElementById('winning').textContent = "It took you "+ moves.textContent+" moves to finish the game!";
            document.getElementById('winning2').textContent = "You received a score of "+remove.length+"/3 stars!"
            document.getElementById('winning3').textContent = "Your time was "+seconds+" seconds! Click the restart to play again!";
          }, 1500);
        }
      }
    }
